...
| Heatmap is all zeros even with data | Time block question wording must START with "Morning" or "Afternoon". Heatmap uses wildcard match (`Morning*` / `Afternoon*`) so anything starting with those words works. If your options use different wording, edit the formulas on Dashboard rows 9–12. |
